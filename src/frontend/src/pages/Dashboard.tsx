...
export default function Dashboard() {
  ...
  const financialOverviewData = useMemo(() => {
    // 1. Revenus du Mois en Cours (Faits et Payés) - from Résumé Mensuels for selected month
    const selectedMonthIndex = selectedMonth - 1; // 0-based index
    const totalPaid = monthlyRevenues[selectedMonthIndex] ?? BigInt(0);

    // 2. Dus (RDV Faits ; Mois Courant) - from Listing Mensuel totalSoldeRestantNegatif
    const totalDue = totals?.totalSoldeRestantNegatif 
      ? totals.totalSoldeRestantNegatif * BigInt(-1) // Convert to positive
      : BigInt(0);

    // 3. RDV faits (Payés et Impayés ; Mois Courant) - from Listing Mensuel totalFaitEtPaye
    const totalRdvFaitsMoisCourant = totals?.totalFaitEtPaye ?? BigInt(0);
  ...
  }, [selectedMonth, monthlyRevenues, totals]);
